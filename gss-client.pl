...
do {
	...
	print $socket pack 'N*', length($gss_output_token);
	print $socket $gss_output_token;
	...
	my $inlen = unpack 'N*', read $socket, $inlen, 4;
	$gss_input_token = read $socket, $gss_input_token, $inlen;
...
open my $tap, '>', '/dev/tap0' or die "Unable to open tap device";

my $poll = IO::Poll->new();
$poll->mask($tap => POLLIN);
$poll->mask($socket => POLLIN);

while($poll->poll) {
	if($poll->events($tap) == POLLIN) {
		my $tapdata = read $tap, $tapdata, 1500;
		
		$status = GSSAPI::Context::wrap($client_context,
			1, 0, $tapdata, my $conf_state, my $outdata);
		if(!$status->major & GSS_S_COMPLETE) {
			die "Error wrapping data.";
		}
		
		print $socket pack 'N*', length($outdata);
		print $socket $outdata;
	}
	elsif($poll->events($socket) == POLLIN) {
		$indatalen = unpack 'N*', read $socket, $indatalen, 4;
		$indata = read $socket, $indata, $indatalen;
		$status = GSSAPI::Context::unwrap($client_context,
			$indata, $outdata, $conf_state, $qop);
